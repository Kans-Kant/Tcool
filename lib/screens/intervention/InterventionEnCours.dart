...
import 'SuivreIntervention.dart';

class InterventionEnCours extends StatefulWidget {
  @override
  _InterventionEnCoursState createState() => _InterventionEnCoursState();
}

class _InterventionEnCoursState extends State<InterventionEnCours> {
  Widget progressIndicator = CircularProgressIndicator();
  List<Intervention> interventionEnCours = List();
  ...
  getMyInterventions() {
    ...
    if (mounted) {
      ...
      InterventionController()
          .getInterventionsInProgessOfOwner(context)
          .then((value) {
        if (mounted) {
          setState(() {
            interventionEnCours = value;
            filteredInterventions = interventionEnCours;
          });
        }
      })
  ...
  getAllInterventions() {
    isLoading =true;
    InterventionController().getAllInterventionsEnCours(context).then((value) {
      if (mounted) {
        setState(() {
          interventionEnCours = value;
          filteredInterventions = interventionEnCours;
