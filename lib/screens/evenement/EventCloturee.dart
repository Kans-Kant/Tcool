...
class EventCloturee extends StatefulWidget {
  @override
  _EventClotureeState createState() => _EventClotureeState();
}

class _EventClotureeState extends State<EventCloturee> {
  Widget progressIndicator = CircularProgressIndicator();
  List<Evenement> eventsCloturees = List();
  ...
  getMyEvents() {
    if (mounted) {
      isLoading = true;
      EventController()
          .getEventsWithOwnerAndStatus(context, "CLOTUREE")
          .then((value) {
        if (mounted) {
          setState(() {
            eventsCloturees = value;
            filteredEvents = eventsCloturees;
          });
        }
      })
  ...
  getAllEvents() {
    if (mounted) {
      isLoading = true;
      EventController().getAllEventsCloturees(context).then((value) {
        setState(() {
          eventsCloturees = value;
          filteredEvents = eventsCloturees;
        });
