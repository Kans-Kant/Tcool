...
class EventEnCours extends StatefulWidget {
  @override
  _EventEnCoursState createState() => _EventEnCoursState();
}

class _EventEnCoursState extends State<EventEnCours> {
  Widget progressIndicator = CircularProgressIndicator();
  List<Evenement> eventEnCours = List();
  ...
  getMyEvents() {
    if (mounted) {
      isLoading =true;
      EventController()
          .getEventsInProgessOfOwner(context)
          .then((value) {
        if (mounted) {
          setState(() {
            eventEnCours = value;
            filteredEvents = eventEnCours;
          });
        }
      })
  ...
  getAllEvents() {
    if (mounted) {
      isLoading = true;
      EventController().getAllEventsEnCours(context).then((value) {
        setState(() {
          eventEnCours = value;
          filteredEvents = eventEnCours;
        });
